...
if(isset($_GET['pacientes'])){
    ...
    $estado         = GETPOST('estadoslist');
    if(!empty(GETPOST('date'))){
        $fechaInicio    = str_replace('/', '-', (explode('-', GETPOST('date'))[0]));
        $fechaFin       = str_replace('/', '-', (explode('-', GETPOST('date'))[1]));
    }else{
        $fechaInicio    = "";
        $fechaFin       = "";
    }
    $paciente       = GETPOST('pacientes');
    $n_citas        = GETPOST('ncita');
...
//    echo '<pre>';  print_r($sql); die();
    ...
    $getNompaciente = getnombrePaciente($paciente)->nom;

    $objPHPExcel->setActiveSheetIndex(0)->setCellValueExplicit('A1', 'REPORTE DE CITAS AGENDADES DEL PACIENTE '.strtoupper($getNompaciente) . date('Y/m/d'));
...
//        $valueEx[] = $items['paciente'];
        ...
        $valueEx[] = ((count($tto)>0)?implode(', ', $tto): "");
        $itemsAgenda[] = $valueEx;
    }
    ...
    header('Content-Disposition: attachment;filename="Reporte CITA '.(!empty($getNompaciente)?strtoupper($getNompaciente):"").'.xlsx"');
